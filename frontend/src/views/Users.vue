...
import { useUserAdminStore } from '../store/modules/userAdmin.store'
import { isMessageBoxDismiss } from '../utils/elMessageBox'
import { notifyError, notifySuccess, notifyWarning } from '../utils/notify'
import type { UserListItem } from '../api/users'

const userStore = useUserStore()
const adminStore = useUserAdminStore()

const { list, loading, batchLoading, rowDeletingId } = storeToRefs(adminStore)

onMounted(() => {
  void adminStore.fetchList()
})
...
async function handleDelete(row: UserListItem) {
  try {
    ...
    await adminStore.removeById(row.id)
    notifySuccess('已删除')
  } catch (e) {
...
async function handleBatchDelete() {
  ...
  const ids = adminStore.deletableIds(userStore.userInfo?.id)
  if (ids.length === 0) {
  ...
  try {
    await ElMessageBox.confirm(
      `将删除 ${ids.length} 个用户（已排除当前登录账号），是否继续？`,
      '批量删除',
      { type: 'warning' },
    )
    const { success, failed } = await adminStore.removeMany(ids)
    if (failed > 0) {
      notifyError(`删除完成：成功 ${success}，失败 ${failed}`)
    } else {
      notifySuccess(`已删除 ${success} 个用户`)
    }
  } catch (e) {
    if (isMessageBoxDismiss(e)) return
    notifyError('批量删除失败')
      ...
      <template #header>
        <div class="card-head">
          ...
          <div class="card-actions">
            <el-button size="small" @click="adminStore.fetchList()">刷新</el-button>
